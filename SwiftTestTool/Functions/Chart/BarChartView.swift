...
//
// https://www.devtechie.com/community/public/posts/279815-chart-interaction-using-chartoverlay-in-swiftui-and-charts-framework
...
struct PracticeChartView: View {
    ...
    @State private var data = Workout.swimData
    ...
    init(data: [Workout] = Workout.swimData) {
    ...
    var body: some View {
        NavigationStack {
            Chart(data) {
                BarMark(x: .value("Day", $0.day), y: .value("Minutes", $0.minutes))
            }
            .frame(height: 400)
                ...
                GeometryReader { geoProxy in
                    ...
                    Rectangle()
                        ...
                        .gesture(DragGesture().onChanged { value in
                            ...
                            let (day, _) = overlayProxy.value(at: location, as: (String, Int).self) ?? ("-", 0)
                            let mins = Workout.swimData.first { w in
        ...
        .navigationTitle("Chart 2")
    }
}

// MARK: Preview
#Preview {
    PracticeChartView(data: Workout.swimData)
}
